...
0. **Iteration 0 — description と body の整合チェック**（静的、agent dispatch 不要）
   ...
   - これを飛ばすと、別エージェントは description に合わせて body を「再解釈」し、実質 skill が要件を満たしていないのに精度が出る（false positive）
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。エージェント起動機構で **新規の別エージェントを起動** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。起動不能環境の扱いは「環境制約」節を参照。
3. **実行**: 後述の **別エージェント起動契約** に従ったプロンプトを別エージェントに渡し、シナリオを実行させる。実行者は実装や出力を生成し、最後に自己申告レポートを返す。
4. **両面評価**: 戻ってきた結果から次を記録する。
   - **実行者の自己申告**（別エージェントのレポート本文から抽出）: 不明瞭点 / 裁量補完 / テンプレ適用で詰まった箇所
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（エージェント実行結果に付く usage メタの `tool_uses` をそのまま使う。読み取り系ツール呼び出しも含める、除外しない）
     - 所要時間（エージェント実行結果の usage メタの `duration_ms`）
     - 再試行回数（別エージェントが同じ判断をやり直した回数。自己申告レポートから抽出、指示側では測れない）
...
6. **再評価**: 新しい別エージェントで再度 2 → 5 を回す（同一 agent は再利用しない: 前回の改善を学習している）。並列度はイテレーションを進めても改善が頭打ちにならない場合に増やす。
...
これを安定させるには **差分適用前に別エージェントへ「この修正が判定文言のどれを満たすか」を言語化させる**。閾値文言レベルで紐付けないと見積もり精度が出ない。評価軸を新設するときも、各点の判定基準を閾値文言レベルまで具体化しておくこと（「全部明示」「動く最小構成全文」のように、何があれば 2 点になるか別エージェントが判定できる粒度）。
...
## 対象プロンプト
<対象プロンプトの本文を全文貼る or 対応するファイル参照手段で読ませるパスを指定>
...
呼び出し側はレポートから自己申告部分を抽出し、`tool_uses` / `duration_ms` をエージェント実行結果の usage メタから取得して評価軸表を埋める。

## 環境制約

新規の別エージェントを起動できない環境（既に別エージェントとして動作している、エージェント起動機構が無効化されている等）では、本 skill は **適用しない**。
- 代替案 1: 親セッションのユーザーに別エージェントセッションを起動して依頼してもらう
- 代替案 2: 評価を諦め、ユーザーに「empirical evaluation skipped: agent dispatch unavailable」と明示報告する
- **NG**: 自己再読で代替する（バイアスが入るので評価結果を信じてはいけない）

**構造審査モード**: empirical 評価ではなく、skill / プロンプトの **記述の整合性・明瞭性だけ** をチェックしたい場合は、構造審査モードとして明示的に切り分ける。別エージェントへの依頼プロンプトに「今回は構造審査モード: 実行ではなくテキスト整合性チェック」と明記する。これにより別エージェントは環境制約節の skip 動作に引っかからず、静的レビューを返せる。構造審査は empirical の代替ではなく補助（連続クリア判定には使えない）。
...
| 出てくる合理化 | 実態 |
|---|---|
| 「自分で読み直せば同じ効果がある」 | 直前に書いた文章を "客観視" はできない。必ず新規の別エージェントを起動する。 |
...
| 「同じ別エージェントを使い回そう」 | 前回の改善を学習している。毎回新規に起動する。 |
...
- `superpowers:writing-skills` — skill 作成時の TDD アプローチ。本 skill の「別エージェントで baseline → 修正 → 再実行」と本質的に同じ
